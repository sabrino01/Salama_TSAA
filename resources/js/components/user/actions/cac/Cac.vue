...
const actionsCac = ref([]);
const sourcesCac = ref([]);
const typeActionsCac = ref([]);
const responsables = ref([]);
const suivis = ref([]);
const constatsCac = ref([]);
const users = ref([]);
...
const chargerActions = async (page = 1, search = "") => {
    try {
        if (paginationEnabled.value) {
            ...
            const response = await axios.get("/api/actions/cac", {
                params: { page, search },
            });
            actionsCac.value = response.data.data;
            ...
            actionsCac.value = allActions.value;
            totalActions.value = allActions.value.length;
            currentPage.value = 1;
            lastPage.value = 1;
        }
    } catch (error) {
        toast.error("Erreur lors du chargement du CAC ou de l'action", error);
    }
};
...
const verifierTousLesStatuts = async () => {
    ...
    actionsCac.value.statut = result;
...
const chargerSourcesCac = async () => {
    try {
        const response = await axios.get("/api/sourcesCac");
        sourcesCac.value = response.data;
    } catch (error) {
        console.error("Erreur lors du chargement des sources CAC:", error);
    }
};

const chargerTypeActionsCac = async () => {
    try {
        const response = await axios.get("/api/typeactionsCac");
        typeActionsCac.value = response.data;
    } catch (error) {
        console.error(
            "Erreur lors du chargement des types d'actions CAC:",
            error
        );
    }
};
...
const chargerConstatsCac = async () => {
    try {
        const response = await axios.get("/api/constatsCac");
        constatsCac.value = response.data;
    } catch (error) {
        console.error("Erreur lors du chargement des constats CAC:", error);
    }
};
...
const supprimerAction = async (id) => {
    toast.confirm(
        "Êtes-vous sûr de vouloir supprimer cette action pour CAC ?",
        async () => {
            try {
                await axios.delete(`/api/actions/${id}`);
                toast.success("Action CAC supprimée avec succès");
                chargerActions(currentPage.value, searchQuery.value);
            } catch (error) {
                toast.error(
                    "Erreur lors de la suppression de l'action du CAC",
                    error
                );
            }
        }
...
const formattedActions = computed(() => {
    return actionsCac.value.map((action) => {
...
const voirCac = (id) => {
    router.push(`/user/actions/cac/voir/${id}`);
};

const voirCacOther = (id) => {
    router.push(`/user/actions/cac/voir/other/${id}`);
};

const editerCac = (id) => {
    router.push(`/user/actions/cac/editer/${id}`);
};
...
const actions = [
    {
        label: "Voir",
        class: "text-blue-500",
        handler: (row) => voirCac(row.id),
    },
    {
        label: "Editer",
        class: "text-green-500",
        handler: (row) => editerCac(row.id),
    },
    {
        label: "Supprimer",
        class: "text-red-500",
...
const filterActions = (row, actions) => {
    ...
    if (row.nom_utilisateur === utilisateurConnecte) {
        ...
        return actions
            ...
            .map((action) => ({
                ...action,
                handler: () => voirCacOther(row.id), // Remplacer le handler
            }));
    }
};
...
const exportToPdf = async () => {
    ...
    try {
        ...
        const fileName = `CAC_${new Date().toISOString().split("T")[0]}.pdf`;
        doc.save(fileName);
...
// Fonction pour extraire le numéro d'un num_actions "CAC-0004" => 4
const extractNumber = (str) => {
    const match = str.match(/CAC-(\d+)/);
...
const generateExcelFile = async (
    ...
    const sheet = workbook.addWorksheet("CAC");
    ...
    for (let i = 0; i < additionalRows; i++) {
        ...
        if (i === 0) {
            ...
            sheet.getCell(`A${rowIndex}`).value = {
                formula: `IF(B${rowIndex}<>"", CONCATENATE("CAC-", TEXT(${
                    currentNum + 1
                }, "0000")), "")`,
            };
        } else {
            ...
            sheet.getCell(`A${rowIndex}`).value = {
                formula: `IF(B${rowIndex}<>"", IF(B${
                    rowIndex - 1
                }<>"", CONCATENATE("CAC-", TEXT(VALUE(MID(A${
                    rowIndex - 1
                },4,4))+1, "0000")), CONCATENATE("CAC-", TEXT(${
                    currentNum + 1
                }, "0000"))), "")`,
            };
        }
    ...
    try {
        ...
        saveAs(new Blob([buffer]), "CAC.xlsx");
                ...
                <div class="p-5">
                    ...
                    <div class="min-h-[800px]">
                        ...
                        <div class="flex w-full mt-5 ml-4">
                            <!-- Bouton d'ajout -->
                            <router-link to="/user/actions/cac/ajouter">
                                <button
