...
const chargerConstatsAI = async () => {
    try {
        const response = await axios.get("/api/constatsAI");
        constatsAI.value = response.data;
    } catch (error) {
        console.error("Erreur lors du chargement des constats AI:", error);
    }
};
...
const voirAuditInterneUserOther = (id) => {
    router.push(`/user/actions/auditinterne/voir/other/${id}`);
};

const editerAuditInterne = (id) => {
    router.push(`/user/actions/auditinterne/editer/${id}`);
};
...
const filterActions = (row, actions) => {
    ...
    if (row.nom_utilisateur === utilisateurConnecte) {
        ...
        return actions
            ...
            .map((action) => ({
                ...action,
                handler: () => voirAuditInterneUserOther(row.id), // Remplacer le handler
            }));
    }
};
...
// Fonction pour extraire le numéro d'un num_actions "AI-0004" => 4
const extractNumber = (str) => {
    const match = str.match(/AI-(\d+)/);
...
const generateExcelFile = async (
    ...
    for (let i = 0; i < additionalRows; i++) {
        ...
        if (i === 0) {
            ...
            sheet.getCell(`A${rowIndex}`).value = {
                formula: `IF(B${rowIndex}<>"", CONCATENATE("AI-", TEXT(${
                    currentNum + 1
                }, "0000")), "")`,
            };
        } else {
            ...
            sheet.getCell(`A${rowIndex}`).value = {
                formula: `IF(B${rowIndex}<>"", IF(B${
                    rowIndex - 1
                }<>"", CONCATENATE("AI-", TEXT(VALUE(MID(A${
                    rowIndex - 1
                },4,4))+1, "0000")), CONCATENATE("AI-", TEXT(${
                    currentNum + 1
                }, "0000"))), "")`,
            };
        }
    ...
    try {
        ...
        saveAs(new Blob([buffer]), "Audit_Interne.xlsx");
                ...
                <div class="p-5">
                    ...
                    <div class="min-h-[800px]">
                        ...
                        <div class="flex w-full mt-5 ml-4">
                            <!-- Bouton d'ajout -->
                            <router-link
                                to="/user/actions/auditinterne/ajouter"
                            >
                                <button
