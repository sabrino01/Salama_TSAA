...
const supprimerAction = async (id) => {
    toast.confirm(
        "Êtes-vous sûr de vouloir supprimer cette action pour Audit Interne Inspection ?",
        async () => {
            try {
                await axios.delete(`/api/actions/${id}`);
                toast.success(
                    "Action Audit Interne Inspection supprimée avec succès"
                );
                chargerActions(currentPage.value, searchQuery.value);
            } catch (error) {
                toast.error(
                    "Erreur lors de la suppression de l'action de l'Audit Interne Inspection",
                    error
                );
            }
        }
...
const formattedActions = computed(() => {
    return actionsAII.value.map((action) => {
...
const voirAuditInterneInspection = (id) => {
    router.push(`/user/actions/aii/voir/${id}`);
};

const voirAuditInterneInspectionOther = (id) => {
    router.push(`/user/actions/aii/voir/other/${id}`);
};

const editerAuditInterneInspection = (id) => {
    router.push(`/user/actions/aii/editer/${id}`);
};
...
const exportToPdf = async () => {
    ...
    try {
        ...
        const fileName = `Audit_Interne_Inspection_${
...
const generateExcelFile = async (
    ...
    const sheet = workbook.addWorksheet("Audit Interne");
                ...
                <div class="p-5">
                    ...
                    <div class="min-h-[800px]">
                        ...
                        <div class="flex w-full mt-5 ml-4">
                            <!-- Bouton d'ajout -->
                            <router-link to="/user/actions/aii/ajouter">
                                <button
