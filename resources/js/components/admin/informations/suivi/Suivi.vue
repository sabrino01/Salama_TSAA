...
const chargerSuivi = async (page = 1, search = "") => {
    try {
        const response = await axios.get("/api/suivi", {
            params: { page, search },
        });
...
const changerPage = (page) => {
    if (page >= 1 && page <= lastPage.value) {
        ...
        chargerSuivi(page, searchQuery.value);
    }
};

// Fonction pour gérer la recherche
const rechercherSuivi = () => {
...
const supprimerSuivi = async (id) => {
    toast.confirm("Êtes-vous sûr de vouloir supprimer ce suivi ?", async () => {
        try {
            await axios.delete(`/api/suivi/${id}`);
            toast.success("Suivi supprimé avec succès");
            chargerSuivi(currentPage.value, searchQuery.value);
        } catch (error) {
            toast.error("Erreur lors de la suppression du Suivi", error);
        }
    });
};

const voirSuivi = (id) => {
    router.push(`/admin/informations/suivi/voir/${id}`);
};

const editerSuivi = (id) => {
    router.push(`/admin/informations/suivi/editer/${id}`);
};
...
const actions = [
    {
        label: "Voir",
        class: "text-blue-500",
        handler: (row) => voirSuivi(row.id),
    },
    {
        label: "Editer",
        class: "text-green-500",
        handler: (row) => editerSuivi(row.id),
    },
    {
        label: "Supprimer",
        class: "text-red-500",
        handler: (row) => supprimerSuivi(row.id),
    },
];
        ...
        <div class="flex-1 flex flex-col ml-64">
            ...
            <div class="flex-1 p-5 bg-gray-50 pb-16">
                ...
                <div class="flex w-full mt-5 ml-4">
                    <!-- Bouton d'ajout -->
                    <router-link to="/admin/informations/suivi/ajouter">
                        <button
