...
const allDataEnCours = ref([]); // Toutes les données pour les alertes
const allDataEnRetard = ref([]); // Toutes les données pour les alertes
...
const getFrequenceBorderClass = (frequence) => {
    ...
    // Cas spéciaux : string ou types sans alerte
    if (
        !frequenceObj ||
    ...
    if (infoDebut.doitAlerter || infoSuivis.doitAlerter) {
        ...
        if (joursRestants > 0) {
            return joursRestants <= 7
                ? "border-yellow-100" // Proche dans moins de 7 jours
                : "border-green-400"; // À venir au-delà de 7 jours
        }
        // Si joursRestants < 0 → en retard
        return retardJours <= 7
            ? "border-orange-400" // Retard de moins de 7 jours
            : "border-red-400"; // Retard de plus de 7 jours
    }
...
onMounted(async () => {
    if (userId.value) {
        ...
        await fetchAllData(true); // Vérifier les alertes au chargement initial
    } else {
