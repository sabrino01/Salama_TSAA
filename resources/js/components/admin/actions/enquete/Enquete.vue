...
const actionsEnquete = ref([]);
const sourcesEnquete = ref([]);
const typeActionsEnquete = ref([]);
const responsables = ref([]);
const suivis = ref([]);
const constatsEnquete = ref([]);
const users = ref([]);
...
const chargerActions = async (page = 1, search = "") => {
    try {
        if (paginationEnabled.value) {
            ...
            const response = await axios.get("/api/actions/enquete", {
                params: { page, search },
            });
            actionsEnquete.value = response.data.data;
            ...
            actionsEnquete.value = allActions.value;
        ...
        toast.error(
            "Erreur lors du chargement de l'Enquête de Satisfaction ou de l'action",
            error
        );
    }
};
...
const verifierTousLesStatuts = async () => {
    ...
    actionsEnquete.value.statut = result;
...
const chargerSourcesEnquete = async () => {
    try {
        const response = await axios.get("/api/sourcesEnquete");
        sourcesEnquete.value = response.data;
    } catch (error) {
        console.error(
            "Erreur lors du chargement des sources Enquête de Satisfaction:",
            error
        );
    }
};

const chargerTypeActionsEnquete = async () => {
    try {
        const response = await axios.get("/api/typeactionsEnquete");
        typeActionsEnquete.value = response.data;
    } catch (error) {
        console.error(
            "Erreur lors du chargement des types d'actions Enquête de Satisfaction:",
            error
        );
    }
};
...
const chargerConstatsEnquete = async () => {
    try {
        const response = await axios.get("/api/constatsEnquete");
        constatsEnquete.value = response.data;
    } catch (error) {
        console.error(
            "Erreur lors du chargement des constats Enquête de Satisfaction:",
            error
        );
    }
};
...
const supprimerAction = async (id) => {
    toast.confirm(
        "Êtes-vous sûr de vouloir supprimer cette action pour Enquête de Satisfaction ?",
        async () => {
            try {
                await axios.delete(`/api/actions/${id}`);
                toast.success(
                    "Action Enquête de Satisfaction supprimée avec succès"
                );
                chargerActions(currentPage.value, searchQuery.value);
            } catch (error) {
                toast.error(
                    "Erreur lors de la suppression de l'action de l'Enquête de Satisfaction",
                    error
                );
            }
        }
...
const formattedActions = computed(() => {
    let actions = actionsEnquete.value;
...
const voirEnquete = (id) => {
    router.push(`/admin/actions/enquete/voir/${id}`);
};

const voirEnqueteOther = (id) => {
    router.push(`/admin/actions/enquete/voir/other/${id}`);
};

const editerEnquete = (id) => {
    router.push(`/admin/actions/enquete/editer/${id}`);
};
...
const actions = [
    {
        label: "Voir",
        class: "text-blue-500",
        handler: (row) => voirEnquete(row.id),
    },
    {
        label: "Editer",
        class: "text-green-500",
        handler: (row) => editerEnquete(row.id),
    },
    {
        label: "Supprimer",
        class: "text-red-500",
...
const filterActions = (row, actions) => {
    ...
    if (row.nom_utilisateur === utilisateurConnecte) {
        ...
        return actions
            ...
            .map((action) => ({
                ...action,
                handler: () => voirEnqueteOther(row.id), // Remplacer le handler
            }));
    }
};
...
const exportToExcel = async () => {
    ...
    try {
        ...
        await Promise.all([
            chargerSourcesEnquete(),
            chargerTypeActionsEnquete(),
            chargerResponsables(),
            chargerSuivis(),
            chargerConstatsEnquete(),
            chargerUsers(),
        ]);
        ...
        generateExcelFile(
            selectedData,
            sourcesEnquete.value,
            typeActionsEnquete.value,
            responsables.value,
            suivis.value,
            constatsEnquete.value,
            users.value
        );
...
const exportToPdf = async () => {
    ...
    try {
        ...
        const fileName = `Enquête_Satisfaction_${
...
// Fonction pour extraire le numéro d'un num_actions "ES-0004" => 4
const extractNumber = (str) => {
    const match = str.match(/ES-(\d+)/);
...
const generateExcelFile = async (
    ...
    const sheet = workbook.addWorksheet("Enquête de Satisfaction");
    ...
    for (let i = 0; i < additionalRows; i++) {
        ...
        if (i === 0) {
            ...
            sheet.getCell(`A${rowIndex}`).value = {
                formula: `IF(B${rowIndex}<>"", CONCATENATE("ES-", TEXT(${
                    currentNum + 1
                }, "0000")), "")`,
            };
        } else {
            ...
            sheet.getCell(`A${rowIndex}`).value = {
                formula: `IF(B${rowIndex}<>"", IF(B${
                    rowIndex - 1
                }<>"", CONCATENATE("ES-", TEXT(VALUE(MID(A${
                    rowIndex - 1
                },4,4))+1, "0000")), CONCATENATE("ES-", TEXT(${
                    currentNum + 1
                }, "0000"))), "")`,
            };
        }
    ...
    const statutOptions = ["En cours", "En retard", "Réglé", "Non Réglé"];
    ...
    try {
        ...
        saveAs(new Blob([buffer]), "Enquete_Satisfaction.xlsx");
                ...
                <div class="p-5">
                    ...
                    <div class="min-h-[800px]">
                        ...
                        <div class="flex w-full mt-5 ml-4">
                            <!-- Bouton d'ajout -->
                            <router-link to="/admin/actions/enquete/ajouter">
                                <button
