...
const actionsAII = ref([]);
const sourcesAII = ref([]);
const typeActionsAII = ref([]);
const responsables = ref([]);
const suivis = ref([]);
const constatsAII = ref([]);
const users = ref([]);
...
const chargerActions = async (page = 1, search = "") => {
    try {
        if (paginationEnabled.value) {
            ...
            const response = await axios.get("/api/actions/aii", {
                params: { page, search },
            });
            actionsAII.value = response.data.data;
            ...
            actionsAII.value = allActions.value;
        ...
        toast.error(
            "Erreur lors du chargement de l'audit interne et inspection ou de l'action",
            error
        );
    }
};
...
const verifierTousLesStatuts = async () => {
    ...
    actionsAII.value.statut = result;
...
const chargerSourcesAII = async () => {
    try {
        const response = await axios.get("/api/sourcesAII");
        sourcesAII.value = response.data;
    } catch (error) {
        console.error("Erreur lors du chargement des sources AII:", error);
    }
};

const chargerTypeActionsAII = async () => {
    try {
        const response = await axios.get("/api/typeactionsAII");
        typeActionsAII.value = response.data;
    } catch (error) {
        console.error(
            "Erreur lors du chargement des types d'actions AII:",
            error
        );
    }
};
...
const chargerConstatsAII = async () => {
    try {
        const response = await axios.get("/api/constatsAII");
        constatsAII.value = response.data;
    } catch (error) {
        console.error("Erreur lors du chargement des constats AII:", error);
    }
};
...
const supprimerAction = async (id) => {
    toast.confirm(
        "Êtes-vous sûr de vouloir supprimer cette action pour Audit Interne et Inspection ?",
        async () => {
            try {
                await axios.delete(`/api/actions/${id}`);
                toast.success(
                    "Action Audit Interne et Inspection supprimée avec succès"
                );
                chargerActions(currentPage.value, searchQuery.value);
            } catch (error) {
                toast.error(
                    "Erreur lors de la suppression de l'action de l'Audit Interne et Inspection",
                    error
                );
            }
        }
...
const voirAuditInterneInspection = (id) => {
    router.push(`/admin/actions/aii/voir/${id}`);
};

const voirAuditInterneInspectionOther = (id) => {
    router.push(`/admin/actions/aii/voir/other/${id}`);
};

const editerAuditInterneInspection = (id) => {
    router.push(`/admin/actions/aii/editer/${id}`);
};
...
const actions = [
    {
        label: "Voir",
        class: "text-blue-500",
        handler: (row) => voirAuditInterneInspection(row.id),
    },
    {
        label: "Editer",
        class: "text-green-500",
        handler: (row) => editerAuditInterneInspection(row.id),
    },
    {
        label: "Supprimer",
        class: "text-red-500",
...
const filterActions = (row, actions) => {
    ...
    if (row.nom_utilisateur === utilisateurConnecte) {
        ...
        return actions
            ...
            .map((action) => ({
                ...action,
                handler: () => voirAuditInterneInspectionOther(row.id), // Remplacer le handler
            }));
    }
};
...
const exportToPdf = async () => {
    ...
    try {
        ...
        const fileName = `Audit_Interne_Inspection${
...
// Fonction pour extraire le numéro d'un num_actions "AII-0004" => 4
const extractNumber = (str) => {
    const match = str.match(/AII-(\d+)/);
...
const generateExcelFile = async (
    ...
    const sheet = workbook.addWorksheet("Audit Interne Inspection");
    ...
    sheet.columns = [
        ...
        { header: "Actions", key: "description", width: 30 },
    ...
    for (let i = 0; i < additionalRows; i++) {
        ...
        if (i === 0) {
            ...
            sheet.getCell(`A${rowIndex}`).value = {
                formula: `IF(B${rowIndex}<>"", CONCATENATE("AII-", TEXT(${
                    currentNum + 1
                }, "0000")), "")`,
            };
        } else {
            ...
            sheet.getCell(`A${rowIndex}`).value = {
                formula: `IF(B${rowIndex}<>"", IF(B${
                    rowIndex - 1
                }<>"", CONCATENATE("AII-", TEXT(VALUE(MID(A${
                    rowIndex - 1
                },4,4))+1, "0000")), CONCATENATE("AII-", TEXT(${
                    currentNum + 1
                }, "0000"))), "")`,
            };
        }
    ...
    try {
        ...
        if (usersList.length > 0) {
            ...
            addDropdown("L", userLabels, usersList.length > 50, "Users_Ref");
        }

        // Appliquer les listes plus petites normalement
        addDropdown("I", frequenceOptions, false);
        addDropdown("K", statutOptions, false);
    } catch (error) {
    ...
    try {
        ...
        saveAs(new Blob([buffer]), "Audit_Interne_Inspection.xlsx");
                    ...
                    <div class="min-h-[800px]">
                        ...
                        <div class="w-full text-gray-600 mt-5">
                            <p class="indent-4 font-poppins">
                                ...
                                Audit Interne et Inspection, Enquête de
                                Satisfaction et CAC, d'y ajouter et encore plus.
                            </p>
                        </div>

                        <!-- Choix d'action à faire(A.I / PTA / A.E / C.A.C / A.I.I) -->
                        <div
                        ...
                        >
                            ...
                            <!-- Lien AII -->
                            <router-link
                                to="/admin/actions/aii"
                                class="flex items-center justify-center text-black text-2xl font-bold px-4 py-2 rounded-md w-38"
                                :class="{
                                    'border-b-4 border-blue-600':
                                        $route.path === '/admin/actions/aii',
                                }"
                            >
                                AII
                            </router-link>
                        ...
                        <div class="flex w-full mt-5 ml-4">
                            <!-- Bouton d'ajout -->
                            <router-link to="/admin/actions/aii/ajouter">
                                <button
