...
const actionsAE = ref([]);
const sourcesAE = ref([]);
const typeActionsAE = ref([]);
const responsables = ref([]);
const suivis = ref([]);
const constatsAE = ref([]);
const users = ref([]);
...
const chargerActions = async (page = 1, search = "") => {
    try {
        if (paginationEnabled.value) {
            ...
            const response = await axios.get("/api/actions/ae", {
                params: { page, search },
            });
            actionsAE.value = response.data.data;
            ...
            actionsAE.value = allActions.value;
        ...
        toast.error(
            "Erreur lors du chargement de l'audit externe ou de l'action",
            error
        );
    }
};
...
const chargerSourcesAE = async () => {
    try {
        const response = await axios.get("/api/sourcesAE");
        sourcesAE.value = response.data;
    } catch (error) {
        console.error("Erreur lors du chargement des sources AE:", error);
    }
};

const chargerTypeActionsAE = async () => {
    try {
        const response = await axios.get("/api/typeactionsAE");
        typeActionsAE.value = response.data;
    } catch (error) {
        console.error(
            "Erreur lors du chargement des types d'actions AE:",
            error
        );
    }
};
...
const chargerConstatsAE = async () => {
    try {
        const response = await axios.get("/api/constatsAE");
        constatsAE.value = response.data;
    } catch (error) {
        console.error("Erreur lors du chargement des constats AE:", error);
    }
};
...
const supprimerAction = async (id) => {
    toast.confirm(
        "Êtes-vous sûr de vouloir supprimer cette action pour Audit Externe ?",
        async () => {
            try {
                await axios.delete(`/api/actions/${id}`);
                toast.success("Action Audit Externe supprimée avec succès");
                chargerActions(currentPage.value, searchQuery.value);
            } catch (error) {
                toast.error(
                    "Erreur lors de la suppression de l'action de l'Audit Externe",
                    error
                );
            }
        }
...
const voirAuditExterne = (id) => {
    router.push(`/admin/actions/ae/voir/${id}`);
};

const voirAuditExterneUserOther = (id) => {
    router.push(`/admin/actions/ae/voir/other/${id}`);
};

const editerAuditExterne = (id) => {
    router.push(`/admin/actions/ae/editer/${id}`);
};
...
const actions = [
    {
        label: "Voir",
        class: "text-blue-500",
        handler: (row) => voirAuditExterne(row.id),
    },
    {
        label: "Editer",
        class: "text-green-500",
        handler: (row) => editerAuditExterne(row.id),
    },
    {
        label: "Supprimer",
        class: "text-red-500",
...
const filterActions = (row, actions) => {
    ...
    if (row.nom_utilisateur === utilisateurConnecte) {
        ...
        return actions
            ...
            .map((action) => ({
                ...action,
                handler: () => voirAuditExterneUserOther(row.id), // Remplacer le handler
            }));
    }
};
...
// Fonction pour extraire le numéro d'un num_actions "AE-0004" => 4
const extractNumber = (str) => {
    const match = str.match(/AE-(\d+)/);
...
const generateExcelFile = async (
    ...
    const sheet = workbook.addWorksheet("Audit Externe");
    ...
    for (let i = 0; i < additionalRows; i++) {
        ...
        if (i === 0) {
            ...
            sheet.getCell(`A${rowIndex}`).value = {
                formula: `IF(B${rowIndex}<>"", CONCATENATE("AE-", TEXT(${
                    currentNum + 1
                }, "0000")), "")`,
            };
        } else {
            ...
            sheet.getCell(`A${rowIndex}`).value = {
                formula: `IF(B${rowIndex}<>"", IF(B${
                    rowIndex - 1
                }<>"", CONCATENATE("AE-", TEXT(VALUE(MID(A${
                    rowIndex - 1
                },4,4))+1, "0000")), CONCATENATE("AE-", TEXT(${
                    currentNum + 1
                }, "0000"))), "")`,
            };
        }
    ...
    try {
        ...
        saveAs(new Blob([buffer]), "Audit_Externe.xlsx");
            ...
            <div class="flex-1 overflow-y-auto bg-gray-50">
                <div class="p-5">
                    ...
                    <div class="min-h-[800px]">
                        ...
                        <!-- Choix d'action à faire(A.I / PTA / A.E / C.A.C) -->
                        <div
                        ...
                        <div class="flex w-full mt-5 ml-4">
                            <!-- Bouton d'ajout -->
                            <router-link to="/admin/actions/ae/ajouter">
                                <button
                        ...
                        <!-- Tableau pour afficher les données d'audit externe -->
                        <div class="mt-5 ml-4">
                            <Table
                                ref="tableRef"
                                :columns="columns"
