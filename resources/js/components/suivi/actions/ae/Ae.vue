...
const chargerActionsSuivis = async (page = 1, search = "") => {
    try {
        ...
        if (paginationEnabled.value) {
            ...
            const response = await axios.get("/api/suivis-ae", {
                params,
            });

            actionsSuivisAE.value = response.data.data;
            ...
            actionsSuivisAE.value = allActions.value;
            totalActions.value = allActions.value.length;
            currentPage.value = 1;
            lastPage.value = 1;
        }
    } catch (error) {
        console.error("Erreur lors du chargement des Suivis AE:", error);
        toast.error("Erreur lors du chargement des Suivis AE");
    }
};
...
const formattedActions = computed(() => {
    let actions = actionsSuivisAE.value;
...
const editerAuditExterne = (id) => {
    router.push(`/suivi/actions/ae/editer/${id}`);
};
    ...
    <div class="flex h-screen">
        ...
        >
            ...
            <div class="flex-1 p-5 bg-gray-50 pb-16">
                ...
                <!-- Choix d'action à faire(A.I / PTA / CAC / A.E / ENQUETE / SWOT) -->
