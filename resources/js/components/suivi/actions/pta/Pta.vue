...
const actionsSuivisPTA = ref([]);
const totalActions = ref(0);
const currentPage = ref(1);
const searchQuery = ref("");
const perPage = ref(10); // Même valeur que dans votre backend
const lastPage = ref(1);
const tableRef = ref(null); // Référence vers le composant TableSuivi.vue
...
// Fonction pour charger les suivis PTA
const chargerActionsSuivis = async (page = 1, search = "") => {
    try {
        ...
        if (paginationEnabled.value) {
            ...
            const response = await axios.get("/api/suivis-pta", {
                params,
            });

            actionsSuivisPTA.value = response.data.data;
            ...
            actionsSuivisPTA.value = allActions.value;
            totalActions.value = allActions.value.length;
            currentPage.value = 1;
            lastPage.value = 1;
        }
    } catch (error) {
        console.error("Erreur lors du chargement des suivis PTA:", error);
        toast.error("Erreur lors du chargement des suivis PTA");
    }
};
...
const formattedActions = computed(() => {
    let actions = actionsSuivisPTA.value;
...
const editerPTA = (id) => {
    router.push(`/suivi/actions/pta/editer/${id}`);
};
