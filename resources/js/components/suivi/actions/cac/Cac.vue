...
// Fonction pour charger les suivis CAC
const chargerActionsSuivis = async (page = 1, search = "") => {
    try {
        ...
        if (paginationEnabled.value) {
            ...
            const response = await axios.get("/api/suivis-cac", {
                params,
            });

            actionsSuivisCac.value = response.data.data;
            ...
            actionsSuivisCac.value = allActions.value;
            totalActions.value = allActions.value.length;
            currentPage.value = 1;
            lastPage.value = 1;
        }
    } catch (error) {
        console.error("Erreur lors du chargement des Suivis CAC:", error);
        toast.error("Erreur lors du chargement des Suivis cAC");
    }
};
...
const formattedActions = computed(() => {
    let actions = actionsSuivisCac.value;
...
const editerCAC = (id) => {
    router.push(`/suivi/actions/cac/editer/${id}`);
};
