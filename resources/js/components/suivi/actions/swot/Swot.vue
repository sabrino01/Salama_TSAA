...
const chargerActionsSuivis = async (page = 1, search = "") => {
    try {
        ...
        if (paginationEnabled.value) {
            ...
            const response = await axios.get("/api/suivis-swot", {
                params,
            });

            actionsSuivisSWOT.value = response.data.data;
            ...
            actionsSuivisSWOT.value = allActions.value;
            totalActions.value = allActions.value.length;
            currentPage.value = 1;
            lastPage.value = 1;
        }
    } catch (error) {
        console.error("Erreur lors du chargement des Suivis SWOT:", error);
        toast.error("Erreur lors du chargement des Suivis SWOT");
    }
};
...
const formattedActions = computed(() => {
    let actions = actionsSuivisSWOT.value;
...
const editerSWOT = (id) => {
    router.push(`/suivi/actions/swot/editer/${id}`);
};
