...
const chargerActionsSuivis = async (page = 1, search = "") => {
    try {
        ...
        if (paginationEnabled.value) {
            ...
            const response = await axios.get("/api/suivis-ai", {
                params,
            });

            actionsSuivisAI.value = response.data.data;
            ...
            actionsSuivisAI.value = allActions.value;
            totalActions.value = allActions.value.length;
            currentPage.value = 1;
            lastPage.value = 1;
        }
    } catch (error) {
        console.error("Erreur lors du chargement des Suivis AI:", error);
        toast.error("Erreur lors du chargement des Suivis AI");
    }
};
...
const editerAuditInterne = (id) => {
    router.push(`/suivi/actions/auditinterne/editer/${id}`);
};
