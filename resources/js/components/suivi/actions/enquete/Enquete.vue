...
const actionsSuivisEnquete = ref([]);
...
// Fonction pour charger les suivis Enquete
const chargerActionsSuivis = async (page = 1, search = "") => {
    try {
        ...
        if (paginationEnabled.value) {
            ...
            const response = await axios.get("/api/suivis-enquete", {
                params,
            });

            actionsSuivisEnquete.value = response.data.data;
            ...
            actionsSuivisEnquete.value = allActions.value;
        ...
        console.error(
            "Erreur lors du chargement des Suivis Enquête de Satisfaction:",
            error
        );
        toast.error(
            "Erreur lors du chargement des Suivis Enquête de Satisfaction"
...
const formattedActions = computed(() => {
    let actions = actionsSuivisEnquete.value;
...
const editerEnquete = (id) => {
    router.push(`/suivi/actions/enquete/editer/${id}`);
};
