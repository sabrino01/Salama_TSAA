...
const userId = computed(() => user.value?.suivis_id);

// Fonction pour charger les suivis AI
const chargerActionsSuivis = async (page = 1, search = "") => {
    try {
        const params = {
            page,
            search,
        ...
        // Ajouter l'ID du suivi si l'utilisateur a le rôle suivi
        if (user.value?.role === "suivi" && userId.value) {
            params.suivi_id = userId.value;
        }

        if (paginationEnabled.value) {
            // Comportement original avec pagination
            const response = await axios.get("/api/suivis-aii", {
                params,
            });

            actionsSuivisAII.value = response.data.data;
            ...
            actionsSuivisAII.value = allActions.value;
            totalActions.value = allActions.value.length;
            currentPage.value = 1;
            lastPage.value = 1;
        }
    } catch (error) {
        console.error("Erreur lors du chargement des Suivis AII:", error);
        toast.error("Erreur lors du chargement des Suivis AII");
    }
};
...
const rechercherActions = () => {
    ...
    chargerActionsSuivis(currentPage.value, search);
};
...
const togglePagination = () => {
    ...
    chargerActionsSuivis(1, searchQuery.value);
};
...
const formattedActions = computed(() => {
    let actions = actionsSuivisAI.value;
...
const changerPage = (page) => {
    if (page >= 1 && page <= lastPage.value) {
        ...
        chargerActionsSuivis(page, searchQuery.value);
    }
};

const editerAuditInterneInspection = (id) => {
    router.push(`/suivi/actions/aii/editer/${id}`);
};
...
const columns = [
    ...
    { label: "Statut", field: "statut_suivi" },
    {
        label: "Observation",
        field: "observation_suivi",
        classes: "truncate", // Classes Tailwind à appliquer à cette colonne
        isExpandable: true, // Indique que cette colonne a un contenu extensible
        render: (row) => row.observation_suivi || "—",
    },
            ...
            <div class="flex-1 p-5 bg-gray-50 pb-16">
                ...
                <div class="w-full text-gray-600 mt-5">
                    <p class="indent-4 font-poppins">
                        ...
                        PTA, Audit Externe, CAC, Enquête de Satisfaction, SWOT,
                        Audit Interne Inspection.
                    </p>
                </div>

                <!-- Choix d'action à faire(A.I / PTA / A.E / CAC / ENQUETE / SWOT / A.I.I) -->
                <div class="flex w-full mt-5 ml-4 justify-center space-x-4">
                    ...
                    <!-- Lien AII -->
                    <router-link
                        to="/suivi/actions/aii"
                        class="flex items-center justify-center text-black text-2xl font-bold px-4 py-2 rounded-md w-38"
                        :class="{
                            'border-b-4 border-blue-600':
                                $route.path === '/suivi/actions/aii',
                        }"
                    >
                        Audit Interne Inspection
                    </router-link>
