...
onMounted(async () => {
    try {
        // Charger les données de l'action suivi sélectionnée avec le filtrage par suivi
        const params = {};
        if (user?.role === "suivi" && userId) {
            params.suivi_id = userId;
        }

        const actionResponse = await axios.get(`/api/suivis/aii/${actionId}`, {
            params,
        });
        ...
        // S'assurer que observation_suivi est une chaîne vide si null
        if (
            action.value.observation_suivi === null ||
            action.value.observation_suivi === undefined
        ) {
            action.value.observation_suivi = "";
        }
    } catch (error) {
...
const modifierSuivisAII = async () => {
    try {
        // Validation basique
        if (!action.value.statut_suivi) {
        ...
        const dataToUpdate = {
            statut_suivi: action.value.statut_suivi,
            observation_suivi: action.value.observation_suivi || null,
        };
        ...
        await axios.put(`/api/suivis/aii/${actionId}`, dataToUpdate, {
            params: params,
        });

        router.push("/suivi/actions/aii");
        toast.success("Action Audit Interne Inspection modifiée avec succès !");
    } catch (error) {
        ...
        if (error.response) {
            ...
            toast.error(
                "Erreur lors de la modification de l'action Audit Interne Inspection"
            );
        }
    }
};
        ...
        <div class="flex-1 flex flex-col ml-64">
            ...
            <div class="flex-1 p-5 bg-gray-50 pb-16">
                ...
                <div class="flex w-full">
                    ...
                    >
                        Editer Audit Interne Inspection
                    </div>
                    <div class="basis-[2%]">
                        <Info />
                    </div>
                ...
                <div class="w-full text-gray-600 mt-5">
                    <p class="indent-4 font-poppins">
                        Dans cet espace, vous pourriez éditer l'Audit Interne et
                        Inspection pour pouvoir faire des modifications sur
                        l'Audit Interne et Inspection sélectionnée.
                    </p>
                </div>

                <!-- Formulaire de modification d'Audit Interne Inspection -->
                <div class="w-full mt-5">
                    ...
                    <div class="flex w-[60%] items-center mt-5">
                        ...
                        <textarea
                            ...
                            v-model="action.observation_suivi"
                        ></textarea>
                    </div>

                    <div class="flex w-full justify-end mt-6">
                        <router-link to="/suivi/actions/aii"
                            ><button
