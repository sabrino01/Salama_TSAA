...
const actionsResponsablesAII = ref([]);
...
const chargerActionsResponsables = async (page = 1, search = "") => {
    try {
        ...
        if (paginationEnabled.value) {
            ...
            const response = await axios.get("/api/actions-responsables-aii", {
                params,
            });

            actionsResponsablesAII.value = response.data.data;
            ...
            actionsResponsablesAII.value = allActions.value;
        ...
        console.error(
            "Erreur lors du chargement des actions responsables AII:",
            error
        );
        toast.error("Erreur lors du chargement des actions responsables AII");
    }
};
...
const formattedActions = computed(() => {
    let actions = actionsResponsablesAII.value;
...
const editerAuditInterneInspection = (id) => {
    router.push(`/responsable/actions/aii/editer/${id}`);
};
    ...
    <div class="flex h-screen">
        ...
        <div class="flex-1 flex flex-col ml-64">
            ...
            <div class="flex-1 p-5 bg-gray-50 pb-16">
                ...
                <!-- Choix d'action à faire(A.I / PTA / A.E / CAC / Enquête / SWOT / A.I.I) -->
                <div class="flex w-full mt-5 ml-4 justify-center space-x-4">
                    ...
                    <!-- Lien AII -->
                    <router-link
                        to="/responsable/actions/aii"
                        class="flex items-center justify-center text-black text-2xl font-bold px-4 py-2 rounded-md w-38"
                        :class="{
                            'border-b-4 border-blue-600':
                                $route.path === '/responsable/actions/aii',
                        }"
                    >
                        AII
                    </router-link>
