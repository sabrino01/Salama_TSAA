...
const actionsResponsablesCac = ref([]);
...
const chargerActionsResponsables = async (page = 1, search = "") => {
    try {
        ...
        if (paginationEnabled.value) {
            ...
            const response = await axios.get("/api/actions-responsables-cac", {
                params,
            });

            actionsResponsablesCac.value = response.data.data;
            ...
            actionsResponsablesCac.value = allActions.value;
        ...
        console.error(
            "Erreur lors du chargement des actions responsables CAC:",
            error
        );
        toast.error("Erreur lors du chargement des actions responsables CAC");
    }
};
...
const formattedActions = computed(() => {
    let actions = actionsResponsablesCac.value;
...
const editerCAC = (id) => {
    router.push(`/responsable/actions/cac/editer/${id}`);
};
