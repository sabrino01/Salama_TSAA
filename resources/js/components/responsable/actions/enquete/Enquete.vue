...
const actionsResponsablesEnquete = ref([]);
    ...
    try {
        ...
        if (paginationEnabled.value) {
            ...
            const response = await axios.get(
                "/api/actions-responsables-enquete",
                {
                    params,
                }
            );

            actionsResponsablesEnquete.value = response.data.data;
            ...
            actionsResponsablesEnquete.value = allActions.value;
        ...
        console.error(
            "Erreur lors du chargement des actions responsables Enquête de Satisfaction:",
            error
        );
        toast.error(
            "Erreur lors du chargement des actions responsables Enquête de Satisfaction"
...
const formattedActions = computed(() => {
    let actions = actionsResponsablesEnquete.value;
...
const editerEnquete = (id) => {
    router.push(`/responsable/actions/enquete/editer/${id}`);
};
