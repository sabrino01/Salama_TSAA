...
const actionsResponsablesPTA = ref([]);
...
// Fonction pour charger les actions responsables PTA
const chargerActionsResponsables = async (page = 1, search = "") => {
    try {
        ...
        if (paginationEnabled.value) {
            ...
            const response = await axios.get("/api/actions-responsables-pta", {
                params,
            });

            actionsResponsablesPTA.value = response.data.data;
            ...
            actionsResponsablesPTA.value = allActions.value;
        ...
        console.error(
            "Erreur lors du chargement des actions responsables PTA:",
            error
        );
        toast.error("Erreur lors du chargement des actions responsables PTA");
    }
};
...
const formattedActions = computed(() => {
    let actions = actionsResponsablesPTA.value;
...
const editerPTA = (id) => {
    router.push(`/responsable/actions/pta/editer/${id}`);
};
    ...
    <div class="flex h-screen">
        ...
        >
            ...
            <div class="flex-1 p-5 bg-gray-50 pb-16">
                ...
                <!-- Choix d'action à faire(A.I / PTA / A.E / CAC / Enquête / SWOT) -->
