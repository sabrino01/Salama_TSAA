...
const actionsResponsablesAI = ref([]);
...
const chargerActionsResponsables = async (page = 1, search = "") => {
    try {
        ...
        if (paginationEnabled.value) {
            ...
            const response = await axios.get("/api/actions-responsables-ai", {
                params,
            });

            actionsResponsablesAI.value = response.data.data;
            ...
            actionsResponsablesAI.value = allActions.value;
        ...
        console.error(
            "Erreur lors du chargement des actions responsables AI:",
            error
        );
        toast.error("Erreur lors du chargement des actions responsables AI");
    }
};
...
const formattedActions = computed(() => {
    let actions = actionsResponsablesAI.value;
...
const editerAuditInterne = (id) => {
    router.push(`/responsable/actions/auditinterne/editer/${id}`);
};
