...
const actionsResponsablesAE = ref([]);
...
// Fonction pour charger les actions responsables AE
const chargerActionsResponsables = async (page = 1, search = "") => {
    try {
        ...
        if (paginationEnabled.value) {
            ...
            const response = await axios.get("/api/actions-responsables-ae", {
                params,
            });

            actionsResponsablesAE.value = response.data.data;
            ...
            actionsResponsablesAE.value = allActions.value;
        ...
        console.error(
            "Erreur lors du chargement des actions responsables AE:",
            error
        );
        toast.error("Erreur lors du chargement des actions responsables AE");
    }
};
...
const formattedActions = computed(() => {
    let actions = actionsResponsablesAE.value;
...
const editerAuditExterne = (id) => {
    router.push(`/responsable/actions/ae/editer/${id}`);
};
